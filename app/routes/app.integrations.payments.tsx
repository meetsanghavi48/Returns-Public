...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (!entry) {
    return json({ success: false, message: `Unknown payment provider: ${providerKey}` }, { status: 400 });
  }
  ...
  if (intent === "set_default") {
    ...
    return json({ success: true, message: `${entry.displayName} set as default payment provider.` });
  }
