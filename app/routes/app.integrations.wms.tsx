...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (!entry) {
    return json({ success: false, message: `Unknown WMS provider: ${providerKey}` }, { status: 400 });
  }
  ...
  if (intent === "test_stored") {
    try {
      const config = await prisma.wmsConfig.findUnique({
        where: { shop_providerKey: { shop, providerKey } },
      });
  ...
  if (intent === "set_default") {
    ...
    return json({ success: true, message: `${entry.displayName} set as default WMS provider.` });
  }
